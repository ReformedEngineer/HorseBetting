...
specific_horses= [h.split('.')[0] for h in os.listdir('models/specific')]

print(specific_horses)
...
## specific 
data_to_predict['spec_pred']= data_to_predict['predictions']
for h in specific_horses:
    try:
        print(h)
        model_spec= models.load_model(f"models/specific/{h}.h5")
        horse_data = data_to_predict[data_to_predict['HorseName2'].str.lower() == h]
        spec_predictions = model_spec.predict(split_inputs(horse_data, categorical_features, numerical_features))
        spec_predictions = y_scaler.inverse_transform(spec_predictions)
        # Update the 'spec_pred' column for the current horse
        data_to_predict.loc[horse_data.index, 'spec_pred'] = spec_predictions.flatten()
    except:
        pass
  
data_to_predict['spec_predictions_rank'] = data_to_predict.groupby('Id')['spec_pred'].rank(ascending=True)
